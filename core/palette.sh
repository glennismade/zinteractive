...
$(printf "${c}%-11s${r} ${n}%-24s${r} ${d}%s${r}" "[Alias]" "zi_aliases" "Browse ${alias_count} aliases...")"
  ...
  for group in General Workflow Git Nav Picker "AWS/Infra" Dev Cheat Terminal Alias Custom; do
    printf '%s\n' "$filtered_items" | grep "\[${group}" | sort -t']' -k2
  done

  # Any categories not in the predefined order
  known_pattern="General\|Workflow\|Git\|Nav\|Picker\|Alias\|AWS/Infra\|Dev\|Cheat\|Terminal\|Custom"
